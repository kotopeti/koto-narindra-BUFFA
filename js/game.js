...
let jeu = {
  ...
  ecrireScore: function(ctx){
      ...
      ctx.strokeText("Appuyez ESPACE pour mettre en pause", 100, 900);
      ctx.restore();   
  },
  ecrireLevel: function(ctx){
     ctx.save();
     ctx.translate(this.x, this.y);
     let fontSize = 50;
     ctx.font = fontSize + 'px Courier';
     ...
     let sc=this.score;
    if(sc>80){
       ctx.strokeText(" Level:5", 300, 35);
       this.niveauEnCours=5;
    }
    else if(sc>60){
       ctx.strokeText(" Level: 4", 300, 35);
       this.niveauEnCours=4;
    }
    else if(sc>40){
       ctx.strokeText(" Level: 3", 300, 35);
       this.niveauEnCours=3;
    }
    else if(sc>20){
       ctx.strokeText(" Level: 2", 300, 35);
       this.niveauEnCours=2;
    }
    else{
       ctx.strokeText(" Level: 1", 300, 35);
  ...
  ecrireGO: function(ctx){
      ...
      ctx.font = fontSize + 'px Courier';
      ctx.strokeStyle = "white";
      ctx.strokeText("Level atteint: "+this.niveauEnCours , 80, 370);
      ctx.strokeText("Score finale: "+this.score , 80, 300);
      ctx.strokeStyle = "red";
      ctx.font= '60px Courier';
      ctx.strokeText("GAME OVER", 130, 500);
      ctx.strokeStyle = "white";
       ctx.font = '20px Courier';
      ctx.strokeText("Appuyez ESPACE pour recommencer", 100, 600);
      ctx.restore();   
  }
}
